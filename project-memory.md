...
| --- | --- | --- |
| The canonical product brand is `Harness X Harness Task Runner`; both occurrences of `Harness` are required. | The owner confirmed the exact display name on 2026-08-15. Machine identifiers use `harness-x-harness-task-runner`, and the control-plane Worker uses `harness-x-harness-task-runner-control-plane`. | UI text, MCP identity, User-Agent, GitHub App name and bot slug, Worker name, URLs, workflow authorship, tests, and documentation must preserve the full brand. |
| The branded Worker is the only active task-runner control-plane script. | On 2026-08-15, the owning user authorized the branded ChatGPT MCP connection and retrieved the expected public result through `get_task_result`. The Cloudflare scripts API then confirmed the legacy Worker was deleted; its former health endpoint returned `404` while the branded Worker continued to return `ok`. | Operations, GitHub variables, OAuth callbacks, and ChatGPT connections must use `https://harness-x-harness-task-runner-control-plane.ron-huafeng.workers.dev`. A historical task can still report its pre-migration repository snapshot because task data is immutable execution evidence. |
...
| `Harness X Harness Task Runner` is owned by and installed on the `Harness-X-Harness` organization. | On 2026-08-15, an App JWT identified the exact App name and slug `harness-x-harness-task-runner`; repository installation lookup succeeded for `Harness-X-Harness/runner`. The active installation selects all organization repositories and grants `Actions`, `Contents`, and `Pull requests: write` plus `Metadata: read`. | The control plane can resolve an installation token for the runner repository. Workflow dispatch through the branded Worker remains a separate end-to-end acceptance gate. |
...
| 品牌迁移后的 OAuth 历史清理必须按 client、grant 和 token 的关联关系执行，不能默认清空共享 KV。 | 2026-08-15 的生产元数据显示，旧客户端、当前客户端和本地 OAuth probe 共用同一个 `OAUTH_KV` namespace；grant 的非敏感 `clientId` 与 user subject 可以区分归属。精确删除六个旧记录后，当前 client、grant、token 各保留一个，Worker 继续健康。 | 清理前只读取 key metadata 和关联字段，不读取或输出加密属性；按显式 key 集合删除。清空 namespace 会撤销当前 MCP 会话，只有明确接受重新授权时才这样做。 |
| Cloudflare account API can enumerate Durable Object namespaces and object IDs, but it does not expose stored task contents. | Before migration it listed task objects only as `id` plus `hasStoredData`; after the legacy Worker deletion and historical cleanup on 2026-08-15, the branded `TaskObject` namespace contained zero objects. In this application, task content is read through a Worker binding and is protected by either user OAuth on MCP tools or GitHub Actions OIDC on internal callbacks. | A Cloudflare deployment token cannot be treated as an administrative task-read credential. Production acceptance of task status/result requires evidence from the authorized MCP route or the OIDC callback path. |
